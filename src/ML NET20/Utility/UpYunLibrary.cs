...
{
    public class UpYunLibrary
    {
        /*
        <add key="UpYun_bucketname_key" value="UpYun_bucketname_cdn"/>
        <add key="UpYun_bucketname_file" value="gqkzone"/>
        <add key="UpYun_bucketname_cdn" value="cdnzone"/>
        <add key="UpYun_username" value="minglissszone"/>
        <add key="UpYun_password" value="gqkzwy13806480"/>
        <add key="UpYun_ApiDomainKey" value="UpYun_ApiDomain1"/>
        <!-- {默认 v0.api.upyun.com 自动识别, v1.api.upyun.com 电信, v2.api.upyun.com 联通, v3.api.upyun.com 移动} -->
        <add key="UpYun_ApiDomain0" value="v0.api.upyun.com"/>
        <add key="UpYun_ApiDomain1" value="v1.api.upyun.com"/>
        <add key="UpYun_ApiDomain2" value="v2.api.upyun.com"/>
        <add key="UpYun_ApiDomain3" value="v3.api.upyun.com"/>
        */

        //private string UpYun_bucketname_key = ConfigurationManager.AppSettings["UpYun_bucketname_key"];
        //private string UpYun_ApiDomainKey = ConfigurationManager.AppSettings["UpYun_ApiDomainKey"];

        private UpYun upyun;

        private string CdnImgPath = "/img";
        private string LeftLineSperater = "/";
        private string point = ".";

        public UpYunLibrary(string bucketname, string username, string password, string api_domain)
        {
            //string bucketname = ConfigurationManager.AppSettings[UpYun_bucketname_key];
            //string username = ConfigurationManager.AppSettings["UpYun_username"];
            //string password = ConfigurationManager.AppSettings["UpYun_password"];
            //初始化
            upyun = new UpYun(bucketname, username, password);
            //切换 API 接口的域名
            //string api_domain = ConfigurationManager.AppSettings[UpYun_ApiDomainKey];
            upyun.setApiDomain(api_domain);
        }

        /// <summary>
        /// 获取空间占用大小
        /// </summary>
        public int BucketUsage
        {
            get { return upyun.getBucketUsage(); }
        }
        ...
        {
            return upyun.getFolderUsage(url);
        }
        ...
        public byte[] readFile(string path, int MiB)
        {
            return upyun.readFile(path, MiB);
        }

        /// <summary>
        /// 获取文件信息 return Hashtable('type'=> file | folder, 'size'=> file size, 'date'=> unix time) 或 null
        /// ht["type"] ht["size"] ht["date"]
        /// </summary>
        /// <param name="file">/a/test1.jpg</param>
        /// <returns></returns>
        ...
        /// <returns></returns>
        public string getImgPath(string localPath)
        {
            DateTime nowTime = DateTime.Now;
            string yyyyMM = nowTime.ToString("yyyyMM");
            string fileName = nowTime.ToString("yyyyMMddHHmmssffffff");
            string extensionName = getFileExtension(localPath).ToString();
            return CdnImgPath + LeftLineSperater + yyyyMM + LeftLineSperater + fileName + point + extensionName;
        }
        ...
        /// 上传文件 /a/test1.jpg
        /// </summary>
        /// <param name="localPath">/a/test1.jpg</param>
        /// <param name="path"></param>
        /// <returns></returns>
        public bool writeFile(string localPath, string path)
        {
            FileStream fs = new FileStream(localPath, FileMode.Open, FileAccess.Read);
            BinaryReader r = new BinaryReader(fs);
            byte[] postArray = r.ReadBytes((int)fs.Length);
            ////设置待上传文件的 Content-MD5 值（如又拍云服务端收到的文件MD5值与用户设置的不一致，将回报 406 Not Acceptable 错误）
            //upyun.setContentMD5(UpYun.md5_file("c:\\Koala.jpg"));
            return writeFile(path, postArray, true);
        }
        public bool writeFile(string path, byte[] postArray, bool auto_mkDir)
        {
            return upyun.writeFile(path, postArray, auto_mkDir);
        }

        /// <summary>
        /// 删除文件 /a/test1.jpg
        /// </summary>
        /// <param name="path">/a/test1.jpg</param>
        /// <returns></returns>
    ...
    public class UpYun
    {
        ...
        */
        ...
        private HttpWebResponse newWorker(string method, string Url, byte[] postData, Hashtable headers)
        {
            ...
            {
                request.Headers.Add("Authorization", "Basic " + Convert.ToBase64String(new System.Text.ASCIIEncoding().GetBytes(this.username + ":" + this.password)));
            }
            ...
            try
            {
                ...
                **/
                foreach (object hl in response.Headers)
                {
                    string name = (string)hl;
                    if (name.Length > 7 && name.Substring(0, 7) == "x-upyun")
                    {
